...
class StateSet:
    def __init__(self, name: str, state: State):
        ...
        assert isinstance(state, State), str(state) + " is of type:" + str(type(state))

        self.state = name
        self.stablestate = state

        self.states: List[State] = [state]
        self.startstates: List[State] = []
        self.endstates: List[State] = [state]

    def __str__(self):
        return self.state + ': ' + ', '.join(str(e) for e in self.states)
